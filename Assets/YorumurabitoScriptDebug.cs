...
public class YorumurabitoScriptDebug : MonoBehaviour
{
    enum MurabitoState
    {
        Normal,
    ...
    void OnTriggerStay(Collider other)
    {
        ...
        {
            //村人の裏取り判定の撤去
            GameObject.Find("MurabitoYoru").transform.Find("EnemyBehind").gameObject.SetActive(false);
            //村人のダウン状態のエリアのアクティベート
            GameObject.Find("MurabitoYoru").transform.Find("EnemyDownArea").gameObject.SetActive(true);
        ...
        if (other.gameObject.tag == "HikizuriPosition" && Input.GetKeyDown("v"))
        {
            ...
            {
                GameObject.Find("MurabitoYoru").transform.Find("EnemyDownArea").gameObject.SetActive(false);
            ...
            else if (murabitoState == MurabitoState.Hikizurare)
            {
                ...
                this.gameObject.tag = "MurabitoYoru";
            }
        }
    }
}
